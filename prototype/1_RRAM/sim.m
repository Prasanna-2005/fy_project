...
% SIM  Run one full mission timeline under the RRAM policy.
% strategy: 'rram' (Section 1 of BDTR_Baseline_Methods.md)
%
% Implements the Random Resource Allocation baseline:
%   1. Per-task random draw from capable UAVs (capability check only,
%      ignoring queue load/capacity constraints).
%   2. Deterministic PRNG using Knuth''s multiplicative hash (Section 5.2):
%      idx = mod(task_id * 2654435761, |U_cap|) + 1
%   3. Reactive trigger on capability loss:
%      re-randomizes any task on degraded UAV that lost feasibility among capable UAVs.
%   4. Time-stepped flight and execution matching canonical scenario.
