...
% SIM  State-Aware Bidirectional Reallocation (SABR).
...
for t = 0:cfg.sim.dt:cfg.sim.tEnd
    ...
    if ~isempty(poolIdx)
        ...
        if justChanged
            assignedAfter = find(strcmp({tasks.status}, 'assigned'));
            newlyAssigned = setdiff(assignedAfter, assignedBefore);
            poolLeft = setdiff(poolIdx, newlyAssigned);
            while ~isempty(newlyAssigned) && ~isempty(poolLeft)
                assignedBefore = assignedAfter;
                [uavs, tasks, ev] = reallocationEvent(uavs, tasks, cfg, t, poolLeft);
                events = [events, ev]; %#ok<AGROW>
                assignedAfter = find(strcmp({tasks.status}, 'assigned'));
                newlyAssigned = setdiff(assignedAfter, assignedBefore);
                poolLeft = setdiff(poolLeft, newlyAssigned);
            end
        end
    end
