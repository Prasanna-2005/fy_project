...
q_c = [];
for idx = 1:numel(healthy_uavs)
    u = healthy_uavs(idx);
    capable_all = true;
    for kIdx = 1:numel(t_remain)
        k = t_remain(kIdx);
        if ~isFeasible(tasks(k).requiredCap, uavs(u).capabilities)
            capable_all = false;
            break;
        end
    end
...
for idx = 1:numel(q_c)
    u = q_c(idx);
    q_trial_len = numel(uavs(u).queue) + numel(t_remain);
    other_u     = setdiff(1:numel(uavs), u);
    other_loads = arrayfun(@(v) numel(v.queue), uavs(other_u));
...
uavs(best_target).queue = [uavs(best_target).queue, t_remain];
if uavs(best_target).assignedTask == 0
    uavs(best_target).assignedTask = t_remain(1);
end
...
for idx = 1:numel(t_remain)
    ...
    evSROM(end+1) = struct('time', t, 'taskIdx', k, 'oldUAV', u_fail, 'newUAV', best_target, ...
        'trigger', 'reactive', 'reason', 'SROM-BlockAssign'); %#ok<AGROW>
end
