...
for t = 0:cfg.sim.dt:cfg.sim.tEnd
    ...
    % ---- Deadline expiry sweep: fail any unassigned/atrisk task that has run out of time
    for j = 1:numel(tasks)
        if (strcmp(tasks(j).status,'unassigned') || strcmp(tasks(j).status,'atrisk')) ...
                && t >= tasks(j).deadline
    ...
    % Dispatch idle UAVs to any 'unassigned' tasks still waiting in the queue
...
queueLen = double(uav.assignedTask ~= 0 && ~isCurrentlyAssignedToThisUAV);
...
for r = 1:nT
    ...
    for c = 1:nU
        ...
        costMat(r, c) = costFunction(t, uavs(i), tasks(j), false, cfg);
    end
end
...
if uavs(i).assignedTask ~= 0 && uavs(i).assignedTask ~= j
    oldTask = uavs(i).assignedTask;
    tasks(oldTask).assignedUAV = 0;
    if strcmp(tasks(oldTask).status, 'assigned')
        tasks(oldTask).status = 'unassigned';
        tasks(oldTask).atriskReason = '';
    end
end
uavs(i).assignedTask = j;
...
function safe = displacedTaskHasSafeAlternative(B, uA, uavs, tasks, cfg, t, uavCommittedThisPhase)
safe = false;
for k = 1:numel(uavs)
    ...
    queueLen = double(uavs(k).assignedTask ~= 0 && uavs(k).assignedTask ~= B);
